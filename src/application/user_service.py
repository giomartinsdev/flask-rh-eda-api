...
from domain.events import (
    ...
    PositionChangedEvent, SalaryChangedEvent, DepartmentChangedEvent, 
    ManagerChangedEvent, EmploymentTypeChangedEvent, UserActivatedEvent,
    UserDeactivatedEvent, UserNameChangedEvent, UserEmailChangedEvent,
    UserPhoneChangedEvent, UserAddressChangedEvent, UserHiredEvent,
    UserPromotedEvent, UserDemotedEvent, UserQueriedEvent,
    UserListQueriedEvent, UserEventsQueriedEvent
)
...
class UserService:
    ...
    def update_user(self, user_id: int, user_data: dict, changed_by: int = None) -> Optional[User]:
        ...
        field_event_map = {
            ...
            'position': lambda old, new: PositionChangedEvent(user_id, old, new, changed_by),
            'salary': lambda old, new: SalaryChangedEvent(user_id, old, new, changed_by),
            'department': lambda old, new: DepartmentChangedEvent(user_id, old, new, changed_by),
            'manager_id': lambda old, new: ManagerChangedEvent(user_id, old, new, changed_by),
            'employment_type': lambda old, new: EmploymentTypeChangedEvent(user_id, old, new, changed_by)
        }
        ...
        if 'is_active' in user_data and user_data['is_active'] != current_user.is_active:
    ...
    def change_position(self, user_id: int, new_position: str, new_salary: float, changed_by: int = None) -> Optional[User]:
        ...
        if new_salary > current_user.salary:
            event = UserPromotedEvent(user_id, current_user.position, new_position, current_user.salary, new_salary)
        elif new_salary < current_user.salary:
            event = UserDemotedEvent(user_id, current_user.position, new_position, current_user.salary, new_salary)
        else:
